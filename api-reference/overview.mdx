...
<Warning>
  This reference is auto-generated from the live server's OpenAPI spec. The
  spec file (`api-reference/openapi.json`) is committed by the
  `publish-openapi.yml` workflow in `no-tickets-service` on every release.
  The set of routes currently registered is intentionally narrow while the
  event-store reset lands — see the [sequencing
