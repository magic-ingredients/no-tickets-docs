...
<Warning>
  The spec file (`api-reference/openapi.json`) is currently a placeholder
  while the cross-repo publish workflow is being wired up — routes will
  appear here once the first sync push from `no-tickets-service` lands.
  The set of routes registered upstream is intentionally narrow while the
  event-store reset settles — see the [sequencing
